...
import static common.ConstantMessages.WATER_NOT_SUITABLE;
import static common.ExceptionMessages.AQUARIUM_NAME_NULL_OR_EMPTY;
...
public abstract class BaseAquarium implements Aquarium {
	...
	public String getInfo() {
		...
		String fishOutput = fish.isEmpty() ? "none" : fish.stream().map(Fish::getName).collect(Collectors.joining(" "));

		return String.format("%s (%s):%n" + "Fish: %s%n" + "Decorations: %d%n" + "Comfort: %d", name,
				this.getClass().getSimpleName(), fishOutput, decorations.size(), calculateComfort());
	}
